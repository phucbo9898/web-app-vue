    ...
    <div class="row px-5">
      ...
      <div class="col-md-6">
        <div class="product-info">
          <h5 class="mb-4" style="color: rgb(60, 153, 240)">
            {{ productDetail.name }}
          </h5>
          <p>
            Trạng thái:
            <span :style="handleQuantity(productDetail.quantity).color">{{
              handleQuantity(productDetail.quantity).text
            }}</span>
          </p>
        </div>
        ...
        <div
          ...
          v-for="product_attribute in productDetail.product_attribute_value"
          :key="product_attribute.id"
        >
          <span
            >{{ product_attribute.attribute.name }} :
            {{ product_attribute.value }}</span
          >
        </div>

        <div class="product-price mt-4">
          <span class="text-danger fs-24 font-weight-bold"
            >{{
              productDetail.price.toLocaleString().replaceAll(".", ",")
            }}VND</span
          >
        </div>
        <hr />
        <div class="product-action">
          <button
            @click="addToFavorite(productDetail)"
      ...
      <div class="d-flex">
        ...
        >
          ...
          >
            <div
              class="ml-10 fs-24"
              :class="selectedTab === tab.value ? 'color-tab' : ''"
            >
              {{ tab.label }}
            </div>
          </div>
